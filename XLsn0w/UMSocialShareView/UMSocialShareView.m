...
@implementation UMSocialShareView
...
+(instancetype)shareViewWithPresentedViewController:(UIViewController *)controller items:(NSArray *)items title:(NSString *)title image:(UIImage *)image urlResource:(NSString *)url{
    UMSocialShareView *view = [[UMSocialShareView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
...
-(void)itemClick:(UIButton *)shareButton {
...
//        }
//    }];
    [self.xlsn0wDelegate handleUMSocialShareEventWithTag:shareButton.tag];
}
